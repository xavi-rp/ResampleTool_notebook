...
Once the data set is available, *ncdf4* and *raster* packages functionalities are used to prepare it for resampling. Raw digital numbers (DN) stored in the netCFD files are used in order to avoid floating point imprecisions when scaling the values in R (R uses double-precision floating-point numbers of the  IEEE-754-Standard, whereas the values in the netCDF are stored as float32). DN values can be scaled into real physical values (PV) aftewards using the the formula PV = DN \* *scale* + *offset*, but keeping in mind that *scale* and *offset* must be those reported in each product documentation. 
...
if (grepl(".nc$", ndvi_files[[1]])){
  ...
  r <- raster(t(nc_data[, seq(dim(nc_data)[2], 1, -1)]))
...
As said before, the physical or real value is computed as digital number * scale + offset, but this applies only for valid pixels.
